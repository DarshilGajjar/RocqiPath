...
class ValisConfig:
    ...
    max_non_rigid_reg_dim_px:     int            = 2048 # unchanged, used as it is mentioned in the research article
...
class WSIRegistrar:
    ...
    def __init__(
        ...
        if self.slide_tgt is not None:
            ...
            logger.info(
                f"[ZOOM] Ref={ref_base:g}x ({ref_source}) -> {target_magnification:g}x"
            )
    ...
    def register_slides(self, method: str = "valis") -> None:
        ...
        if self.method == "valis":
            if not HAS_VALIS:
                raise ImportError(
                    "VALIS is not installed. Install with:  pip install valis-wsi"
                )
        ...
        try:
            ...
            def _warp_hires(slide_valis) -> np.ndarray:
                ...
                try:
                    ...
                    logger.warning(f"[WARN] warp_slide() failed ({exc}), falling back to warp_img()")
            ...
            blend = cv2.addWeighted(
                img_ref.astype(np.float32), 0.5,
                img_tgt_r.astype(np.float32), 0.5,
                0,
            ).astype(np.uint8)
    ...
    def save_aligned_wsi(
        ...
        first scaled from thumbnail-space to the chosen pyramid level, then
        ...
        ``cv2.warpAffine``. The completed canvas is then saved using
        ``valis.slide_io.save_ome_tiff`` — the exact same writer used
        internally by VALIS — so the output OME-TIFF is identical in format,
        metadata, and pyramid structure to a VALIS-registered slide.

        Algorithm:
        1. Scale the 2x3 affine matrix from thumbnail-space to level-space
        using ``scale_factor = orb_scale / ds``.
        ...
        OpenSlide, apply a locally translated affine, and blit the result
        onto a pre-allocated canvas.
        4. Open a SlideReader on the target slide to extract original metadata
        (mpp, channel names, OME-XML).
        5. Build OME-XML via ``slide_io.update_xml_for_new_img`` and write
        the pyramidal OME-TIFF via ``slide_io.save_ome_tiff``.

        Output format:
        - Format      : OME-TIFF (pyramidal, tiled, pyvips-written)
        - Warping     : rigid affine only (ORB contour-shape registration)
        - Compression : deflate (lossless)
        - Metadata    : original mpp + channel names preserved from target slide
        ...
        level : int
            ...
            is required. For VALIS, level 1 is recommended. For ORB, any
            level is safe since the canvas is allocated at that level's
            dimensions (``slide_ref.level_dimensions[level]``).
        ...
        if self.method == "valis":
            ...
            try:
                ...
                if tgt_slide_obj is None:
                    logger.error(f"[ERROR] Could not find target slide '{tgt_key}' in VALIS registrar.")
                    logger.debug(f"[DEBUG] Available slides: {[s.name for s in self._registrar.slide_dict.values()]}")
                    return None
        ...
        # ══════════════════════════════════════════════════════════════════════
            ...
            if self.orb_matrix is None:
                logger.error("[ERROR] save_aligned_wsi: ORB matrix not set. Run register_slides() first.")
                return None
            ...
            # ── 2. Canvas size = reference slide at chosen level ──────────────
            out_w, out_h = self.slide_ref.level_dimensions[level]
            ds           = self.slide_ref.level_downsamples[level]

            # ── 3. Scale affine: thumbnail-space → level-space ────────────────
            # orb_matrix is 3×3 in thumbnail px where 1 thumb px = orb_scale full-res px.
            # 1 level px = ds full-res px  →  scale factor = orb_scale / ds
            scale_factor  = self.orb_scale / ds
            M3            = self.orb_matrix.copy()
            M3[0, 2]     *= scale_factor   # tx
            M3[1, 2]     *= scale_factor   # ty
            M_affine      = M3[:2, :]      # 2×3 for cv2.warpAffine

            # Pre-compute inverse once (reused for every tile)
            inv_M2 = cv2.invertAffineTransform(M_affine)
            inv_M3 = np.vstack([inv_M2, [0, 0, 1]])

            # ── 4. Tile-wise warp onto pre-allocated canvas ───────────────────
            TILE          = 1024
            canvas        = np.zeros((out_h, out_w, 3), dtype=np.uint8)
            tgt_w, tgt_h  = self.slide_tgt.level_dimensions[level]
            n_tiles_x     = (out_w + TILE - 1) // TILE
            n_tiles_y     = (out_h + TILE - 1) // TILE
            total         = n_tiles_x * n_tiles_y

            logger.info(f"[ORB SAVE] Canvas  : {out_w}x{out_h} px  level={level}  ds={ds:.2f}")
            logger.info(f"[ORB SAVE] Affine scale (thumb→level): {scale_factor:.4f}")
            logger.info(f"[ORB SAVE] Output  : {output_path}")
            ...
            with tqdm(
                ...
                for ty in range(n_tiles_y):
                    for tx in range(n_tiles_x):
                        ox1, oy1 = tx * TILE,                   ty * TILE
                        ox2, oy2 = min(ox1 + TILE, out_w),      min(oy1 + TILE, out_h)
                        tw_,  th_ = ox2 - ox1,                  oy2 - oy1

                        # Back-project output tile corners → IHC source region
                        corners = np.array(
                            [[ox1, oy1], [ox2-1, oy1], [ox1, oy2-1], [ox2-1, oy2-1]],
                            dtype=np.float64,
                        ).reshape(-1, 1, 2)
                        src_corners = cv2.perspectiveTransform(corners, inv_M3).reshape(-1, 2)

                        sx1 = max(0,         int(np.floor(src_corners[:, 0].min())))
                        sy1 = max(0,         int(np.floor(src_corners[:, 1].min())))
                        sx2 = min(tgt_w - 1, int(np.ceil(src_corners[:, 0].max())) + 1)
                        sy2 = min(tgt_h - 1, int(np.ceil(src_corners[:, 1].max())) + 1)
                        ...
                        try:
                            patch = self.slide_tgt.read_region(
                                (int(sx1 * ds), int(sy1 * ds)), level, (sx2 - sx1, sy2 - sy1)
                            ).convert("RGB")
                        except Exception as exc:
                            pbar.set_postfix(warn=f"tile ({tx},{ty}) read failed")
                            pbar.update(1)
                            continue

                        # Translate affine so src origin = (sx1, sy1), dst origin = (ox1, oy1)
                        M_local        = M_affine.copy()
                        M_local[0, 2] += ox1 - (M_affine[0, 0] * sx1 + M_affine[0, 1] * sy1)
                        M_local[1, 2] += oy1 - (M_affine[1, 0] * sx1 + M_affine[1, 1] * sy1)

                        canvas[oy1:oy2, ox1:ox2] = cv2.warpAffine(
                            np.asarray(patch), M_local, (tw_, th_),
            ...
            # ── 5. Save via valis.slide_io — exact same path as warp_and_save_slide
            logger.info("[ORB SAVE] Writing OME-TIFF via valis.slide_io …")

            # ── 5a. Open a SlideReader on the target slide ────────────────────
            # Mirrors warp_and_save_slide: slide_io.get_slide_reader(src_f) → cls(src_f)
            # get_slide_reader returns the class, not an instance, so we call it.
            tgt_reader = None
            try:
                tgt_reader_cls = slide_io.get_slide_reader(self.path_tgt, series=None)
                tgt_reader     = tgt_reader_cls(self.path_tgt)
                _              = tgt_reader.metadata   # verify metadata is accessible
            except Exception as exc:
                logger.info(f"[ORB SAVE] WARN: could not open SlideReader ({exc}). "
                      "OME-XML will be auto-generated without original metadata.")
                tgt_reader = None

            # ── 5b. Physical pixel size per level ────────────────────────────
            # Mirrors: pixel_physical_size_xyu = ref_slide.reader.scale_physical_size(level)
            pixel_physical_size_xyu = None
            if tgt_reader is not None:
                try:
                    pixel_physical_size_xyu = tgt_reader.scale_physical_size(level)
                except Exception as exc:
                    logger.info(f"[ORB SAVE] WARN: scale_physical_size failed ({exc}). "
                          "mpp will not be written.")

            # ── 5c. Build OME-XML ─────────────────────────────────────────────
            # update_xml_for_new_img returns an ome_xml *object*; call .to_xml()
            # exactly as warp_and_save_slide does before passing to save_ome_tiff.
            ome_xml = None
            if tgt_reader is not None:
                try:
                    ome_xml_obj = slide_io.update_xml_for_new_img(
                        ...
                        reader                  = tgt_reader,
                        level                   = level,
                        channel_names           = None,          # None → reader supplies names
                        colormap                = slide_io.CMAP_AUTO,
                        pixel_physical_size_xyu = pixel_physical_size_xyu,
                    )
                    ome_xml = ome_xml_obj.to_xml()               # required: returns object not str
                except Exception as exc:
                    logger.info(f"[ORB SAVE] WARN: update_xml_for_new_img failed ({exc}). "
                          "OME-XML will be auto-generated.")
                    ome_xml = None

            # ── 5d. Tile size ─────────────────────────────────────────────────
            # Mirrors: tile_wh = slide_io.get_tile_wh(reader, level, out_shape_wh)
            tile_wh = 512   # safe default
            if tgt_reader is not None:
                try:
                    tile_wh = slide_io.get_tile_wh(
                        reader       = tgt_reader,
                        level        = level,
                        out_shape_wh = (out_w, out_h),
                    )
                except Exception:
                    tile_wh = 512

            # ── 5e. Write — identical call to warp_and_save_slide ────────────
            ...
            logger.info(f"[ORB SAVE] Saved → {output_path}")
            return output_path
    ...
    def _register_orb(self) -> None:
        ...
        def _refine_with_phase_correlation(
            ...
            logger.info(f"[ORB]   Residual correction: Δx={res_dx:.2f}px  Δy={res_dy:.2f}px "
        ...
        self.orb_scale_x = self.orb_scale_y = self.orb_scale = W / tw
        ...
        logger.info(f"[ORB] Thumbnail size: {tw}x{th}  |  scale factor: {self.orb_scale:.2f}×")
        ...
        if not cnts_ref or not cnts_tgt:
            logger.warning("[ORB] WARNING: No tissue contours found. Falling back to phase-correlation only.")
            matched_src, matched_dst = [], []
        else:
            MATCH_THRESH = cfg.get("orb_match_threshold", 1.4)
            matched_src, matched_dst = _match_contours_spatial(
                cnts_ref, cnts_tgt, mask_ref.shape, mask_tgt.shape,
    ...
    def _transform_coords(
        ...
        if self.method == "valis" and self._slide_ref_valis is not None:
            try:
                ...
                warped  = self._slide_ref_valis.warp_xy_from_to(
                    coords, self._slide_tgt_valis
                )
            ...
            # Scale to thumbnail space → apply homography → scale back
            pt   = np.array([[[x / self.orb_scale, y / self.orb_scale]]], dtype=np.float32)
            pt_t = cv2.perspectiveTransform(pt, self.orb_matrix)
            return (
                int(pt_t[0, 0, 0] * self.orb_scale),
                int(pt_t[0, 0, 1] * self.orb_scale),
            )
