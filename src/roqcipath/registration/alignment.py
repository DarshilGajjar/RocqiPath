# -*- coding: utf-8 -*-
"""
roqcipath.registration.alignment
========================
...
registration via VALIS or ORB (via ``roqcipath.registration.core``).
...
──────────────────────────────────────────────────────────────────────────────
    ...
    from roqcipath.registration import run_alignment, AlignmentConfig
...
Integration notes
──────────────────────────────────────────────────────────────────────────────
* Fully integrated into the ``roqcipath`` package; zero standalone script
  dependencies.
* Logging is unified with the ``roqcipath.registration.alignment`` child logger.
* ``ValisConfig`` and ``WSIRegistrar`` are consumed directly from
  ``roqcipath.registration.core`` — no duplication.
...
try:
    from roqcipath.registration.core import ValisConfig, WSIRegistrar
...
DEFAULT_FILENAME_PATTERN: str = (
    r"^(?P<sample_id>.+?)_(?P<marker>he|[a-z0-9]+)(?:\.[^.]+)+$"
)
...
def build_sample_pairs(
    ...
    for sid in sample_ids:
        ...
        if he_path and marker_path:
            ...
            if not he_path:
                logger.warning(
                    f"{sid}: {marker_upper} found but H&E is missing"
                )
            if not marker_path:
                logger.warning(
                    f"{sid}: H&E found but {marker_upper} is missing"
                )
    return pairs
...
class AlignmentProcessor:
    ...
    def __init__(self, config: AlignmentConfig) -> None:
        ...
        self.biomarkers: List[str] = (
            ...
            else discover_biomarker_folders(self.input_dir)
        )
        if not self.biomarkers:
            logger.warning(
                "No biomarker subfolders found under: %s\n"
    ...
    def _make_valis_config(self) -> Any:
        ...
        :class:`~roqcipath.registration.core.WSIRegistrar`.
        ...
        ------
        RuntimeError
            If :mod:`roqcipath.registration.core` (and therefore VALIS)
        ...
        if not WSI_PROCESSING_AVAILABLE:
            raise RuntimeError(
                "roqcipath.registration.core is not installed. "
    ...
    def _make_registrar_cfg(self, output_root: Path, item_name: str) -> dict:
        ...
        dict
            ...
            :class:`~roqcipath.registration.core.WSIRegistrar` for the
    ...
    def run(self) -> List[AlignedCaseResult]:
        ...
        for biomarker in self.biomarkers:
            ...
            if not pairs:
                logger.warning(
                    f"No complete H&E / {biomarker} pairs found in {biomarker_path}"
                )
                continue
