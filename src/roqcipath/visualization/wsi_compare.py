#!/usr/bin/env python3
"""
roqcipath.visualization.wsi_compare
=====================================
...
reports, as opposed to :mod:`roqcipath.visualization.visualization`'s
quick exploratory QC plots.

Also runnable as a standalone script::

    python -m roqcipath.visualization.wsi_compare --help

Uses its own self-contained banner/logging setup (via ``loguru``,
independent of :mod:`roqcipath.logger`) since it predates the unified
logging system and is also usable outside the rest of the package.

WSI Visualization Module for ROQCIPath — PUBLICATION-QUALITY EDITION
...
def _build_banner(tool_name: str, subtitle: str = "") -> str:
    ...
    Rich-based banner in :func:`roqcipath.logger.print_banner` — this
...
def _tissue_fraction(img: Image.Image, bbox: Tuple[int, int, int, int]) -> float:
    ...
    """
    crop  = img.crop(bbox)
    thumb = crop.resize((_REGION_THUMB_SIZE, _REGION_THUMB_SIZE),
    ...
    n_tissue = sum(
        1 for g, s in zip(grey_arr, sat_arr)
        if g  <  _TISSUE_BRIGHTNESS_THRESHOLD
        and s > (_TISSUE_SATURATION_THRESHOLD * 255)
    )
...
def _region_bbox(
    ...
    if region not in anchors:
        raise ValueError(
            f"Unknown region '{region}'. Valid choices: {list(anchors)}"
        )
    ...
    for _, bbox in candidates:
        ...
        if frac >= REGION_TISSUE_THRESHOLD:
            logger.debug(
                f"[{region}] accepted at tissue={frac:.1%}  bbox={bbox}"
            )
            return bbox
...
def _load_or_create_roi_sidecar(
    ...
    if os.path.isfile(sidecar_path):
        ...
        if (
            ...
            logger.warning(
                "Existing ROI sidecar does not match current parameters — regenerating."
            )
...
def _add_scale_bar(
    ...
    locations_map = {
        ...
        "bottom_right": lambda w, h: (w - margin - bar_length_px, h - margin - vertical_space_needed),
    ...
    # Vertical end-ticks
    draw.line([(x,                y - tick_h // 2), (x,                y + tick_h // 2)],
              fill=(255, 255, 255, 255), width=thickness)
    draw.line([(x + bar_length_px, y - tick_h // 2), (x + bar_length_px, y + tick_h // 2)],
              fill=(255, 255, 255, 255), width=thickness)
...
def _save_annotated_full_view(
    ...
    zoom_colors = {
        zl: _ROI_COLORS[i % len(_ROI_COLORS)]
        for i, zl in enumerate(zoom_labels_ordered)
    }

    for zoom_label in zoom_labels_ordered:
        for bbox, roi_id in zip(
            bboxes_per_zoom[zoom_label], roi_ids_per_zoom[zoom_label]
        ):
...
def visualize_side_by_side(
    ...
    pending_named   = [(p, zl, r, s) for p, zl, r, s in named_outputs
    ...
    try:
        ...
        if random_mode:
            roi_data = _load_or_create_roi_sidecar(
                sidecar_path, img_he, w, h, zoom_sizes, n_random_rois, roi_seed,
            )
            ...
            pending_roi   = [(p, zl, rid, bb) for p, zl, rid, bb in roi_outputs
                             if not os.path.isfile(p)]
...
# ============================================================================
def main() -> None:
    """Entry point for ``python -m roqcipath.visualization.wsi_compare``.
    ...
    parser.add_argument("--he",       default=None, dest="he_path",
                        help="Path to the H&E TIFF. Required if --manifest is not provided.")
    parser.add_argument("--gt_ihc",   default=None, dest="gt_ihc_path",
                        help="Path to the GT IHC TIFF. Required if --manifest is not provided.")
    parser.add_argument("--pred_ihc", default=None, dest="pred_ihc_path",
    ...
    parser.add_argument("--title_gt",   default="Ground Truth IHC", help="Title for the GT IHC panel.")
    parser.add_argument("--title_pred", default="Predicted IHC",    help="Title for the Predicted IHC panel.")
    parser.add_argument("--dpi",        type=int, default=600,       help="DPI for saved figures (300+ for print).")
    ...
    if args.manifest:
        ...
        missing = [n for n, v in [("--he", args.he_path),
                                   ("--gt_ihc", args.gt_ihc_path),
                                   ("--pred_ihc", args.pred_ihc_path)] if not v]
        if missing:
            parser.error(
    ...
    configure_logging(
        tool_name="ROQCIPath — Publication-Quality Visualizer",
