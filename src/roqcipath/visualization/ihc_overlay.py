"""
roqcipath.visualization.ihc_overlay
=====================================
...
:class:`roqcipath.analysis.cell_counting.PositiveCellCounter`, generalized
...
::
    ...
    from roqcipath.visualization.ihc_overlay import (
...
from roqcipath.exceptions import ConfigurationError, ExtractionError
...
class MarkerProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.color) != 3 or not all(0 <= c <= 255 for c in self.color):
            raise ConfigurationError(
                f"color must be an (R, G, B) tuple with each value in [0, 255]; "
                f"got {self.color}"
            )
...
class IHCOverlayConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.base_marker not in self.markers:
            raise ConfigurationError(
                f"base_marker {self.base_marker!r} not found in markers: "
        ...
        for combo in self.combinations:
            if combo.base not in self.markers:
                raise ConfigurationError(
                    f"OverlayCombo.base {combo.base!r} not found in markers: "
                    f"{sorted(self.markers)}"
                )
            for ov in combo.overlays:
                if ov not in self.markers:
                    raise ConfigurationError(
                        f"OverlayCombo overlay {ov!r} not found in markers: "
                        f"{sorted(self.markers)}"
                    )
...
def _marker_mask(img_rgb: np.ndarray, profile: MarkerProfile) -> np.ndarray:
    ...
    :meth:`roqcipath.analysis.cell_counting.PositiveCellCounter._brown_mask`
    ...
    thresh_val, _ = cv2.threshold(
        gated_vals, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
...
def _build_composite(images: Dict[str, np.ndarray], masks: Dict[str, np.ndarray],
                     combo: OverlayCombo, cfg: IHCOverlayConfig) -> np.ndarray:
...
def _save_composite_figure(composite: np.ndarray, out_path: str,
                           cfg: IHCOverlayConfig) -> None:
...
def _save_grid_figure(images: Dict[str, np.ndarray], masks: Dict[str, np.ndarray],
                      composite: np.ndarray, combo: OverlayCombo,
                      cfg: IHCOverlayConfig, out_path: str) -> None:
...
def _looks_like_case_dir(path: str, marker_keys: List[str]) -> bool:
    ...
    if not os.path.isdir(path):
        return False
    existing = {n.lower() for n in os.listdir(path)
...
def _process_single_case(case_id: str, case_dir: str,
                         cfg: IHCOverlayConfig) -> Dict[str, Any]:
    ...
    needed_markers = sorted({
        ...
        for key in [combo.base, *combo.overlays]
    })
...
def process_ihc_overlay(
    ...
    **Parallelism.** In batch mode, when ``cfg.max_workers > 1`` and more
    ...
    :func:`roqcipath.extraction.patch_extraction.run_patch_extraction`:
    ...
    if cfg.max_workers > 1 and len(case_dirs) > 1:
        with concurrent.futures.ThreadPoolExecutor(max_workers=cfg.max_workers) as pool:
            futures = {
                pool.submit(_process_single_case, cid, cdir, cfg): cid
