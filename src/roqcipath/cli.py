...
# -*- coding: utf-8 -*-
"""
roqcipath  —  Interactive CLI
====================================
Entry point for the roqcipath whole-slide image (WSI) processing pipeline.

Menu
────
    1. Alignment Pipeline      — register all H&E / IHC pairs under a
                                  directory tree (drives roqcipath.registration).
    2. Core Extraction         — detect and extract tissue cores from paired
                                  multi-region slides (drives roqcipath.extraction).
    3. Stain Normalization     — train or apply Reinhard/Macenko/Vahadane
                                  normalisers (drives roqcipath.stain).
    4. Exit
...
importable API via ``roqcipath.api``.  See the README for examples.
"""
...
try:
    ...
    HAS_ALIGNMENT = False
    logger.warning(f"Alignment module not available: {_e}")
...
try:
    ...
    logger.warning(f"Extraction module not available: {_e}")
...
try:
    ...
    logger.warning(f"Stain normalization module not available: {_e}")
...
def _cmd_alignment() -> None:
    ...
    :func:`roqcipath.registration.run_alignment`.
    ...
    if not HAS_ALIGNMENT:
        logger.error(
            "roqcipath.registration is not available. "
            "Ensure roqcipath.registration.core is installed."
        )
        return
    ...
    raw_bio = input(
        "  Biomarker folders to process (comma-separated, or Enter for all): "
    ).strip()
    biomarker_folders = (
        [b.strip() for b in raw_bio.split(",") if b.strip()] if raw_bio else []
    )
    ...
    valis_max_err     = _get_optional_float(
...
def _cmd_extraction() -> None:
    ...
    :func:`roqcipath.extraction.run_core_extraction_pipeline`.
    """
    if not HAS_EXTRACTION:
        logger.error(
            "roqcipath.extraction is not available. "
            "Ensure pyvips and opencv-python are installed."
        )
        return
...
def _cmd_stain_normalization() -> None:
    ...
    Drives :func:`roqcipath.stain.run_stain_normalization_train`
    and :func:`roqcipath.stain.run_stain_normalization_apply`.
    """
    if not HAS_STAIN_NORM:
        logger.error(
            "roqcipath.stain is not available. "
            "Ensure tiatoolbox is installed."
        )
        return
    ...
    input_dir  = _get_existing_dir("  Input directory: ")
    output_dir = _get_dir("  Output directory: ")
    n_type     = input("  Algorithm — reinhard / macenko / vahadane [macenko]: ").strip().lower() or "macenko"
...
def _print_header() -> None:
    ...
    dynamically from ``roqcipath.__version__`` (rather than a hardcoded
    string) so the displayed version never drifts out of sync with the
    installed package.
    """
    import roqcipath
    print("\n" + _SEP)
    print(f"  roqcipath  |  Author: Darshil Gajjar  |  Version: v{roqcipath.__version__}")
    print(_SEP)
    print("  Main Menu:")
    ...
    print("  grid maps, and visualisation are importable from roqcipath.api")
    print()
...
def main() -> int:
    """
    Entry point for the ``roqcipath`` console script.
