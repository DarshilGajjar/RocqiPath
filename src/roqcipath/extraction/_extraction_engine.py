"""
roqcipath.extraction._extraction_engine
===================================
...
def configure_logging(save_dir: Optional[str] = None, *, file_level: str = "DEBUG",
                      log_filename: str = "extraction.log") -> None:
    ...
    if save_dir is not None:
        ...
        previous = _FILE_SINKS.pop(log_path, None)
        if previous is not None:
            logger.remove(previous)
        _FILE_SINKS[log_path] = logger.add(log_path, level=file_level.upper(),
            format="{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {message}",
            encoding="utf-8", rotation=None)
...
def _vips_to_numpy_rgb(img: Any) -> np.ndarray:
    ...
    """
    if img.hasalpha(): img = img.flatten()
    if img.bands > 3:  img = img[:3]
    return np.ndarray(buffer=img.write_to_memory(), dtype=np.uint8,
                      shape=[img.height, img.width, img.bands])[:, :, :3]
...
def _load_thumbnail(
    ...
    *,
    ...
    if not _PYVIPS_AVAILABLE:
        raise ImportError("pyvips is required. pip install roqcipath[extraction]")
    path = Path(wsi_path)
    ...
    for candidate_level in range(1, 12):
        try:
            ...
            break
        if candidate.width == candidates[-1][1].width and candidate.height == candidates[-1][1].height:
            break
...
def _sort_contours_spatially(contours: List) -> List:
    ...
    rows: List  = []; current_row = [items[0]]
    for item in items[1:]:
        if abs(item[1][1] - current_row[-1][1][1]) < row_tol:
            current_row.append(item)
        else:
            rows.append(current_row); current_row = [item]
    if current_row: rows.append(current_row)
    result: List = []
    for row in rows:
        row.sort(key=lambda i: i[1][0]); result.extend(item[0] for item in row)
    return result


def _detect_regions(thumbnail: np.ndarray, min_area_fraction: float,
                    only_circles: bool = False,
                    min_circularity: float = 0.0) -> List[Dict[str, float]]:
    ...
    for cnt in contours:
        ...
        if only_circles:
            ...
            if peri == 0: continue
            if (4 * np.pi * area) / (peri ** 2) < min_circularity: continue
        valid.append(cnt)
...
def _save_tif(region: Any, tif_path: Path, cfg: _BaseExtractionConfig) -> None:
    ...
    """
    region.tiffsave(str(tif_path), tile=cfg.tif_tile, pyramid=cfg.tif_pyramid,
                    compression=cfg.tif_compression, Q=cfg.tif_quality)
...
def _region_outputs_exist(region_dir: Path, tag: str) -> bool:
    ...
    bool
        ...
        region.
    """
    return all([(region_dir / f"{tag}.tif").exists(),
                (region_dir / f"{tag}_preview.jpg").exists(),
                (region_dir / f"{tag}_manifest.json").exists()])


def _write_region_manifest(manifest_path: Path, *, pipeline: str, sample_id: str,
    region_number: int, source_file: str, rel_box: Dict[str, float],
    abs_box: Dict[str, int], full_slide_dims: Dict[str, int],
    detection_source: str, extra_meta: Optional[Dict[str, Any]] = None) -> None:
    ...
    ``full_slide_dims``.
    """
    m: Dict[str, Any] = {"generated_at": datetime.utcnow().isoformat()+"Z",
        "pipeline": pipeline, "sample_id": sample_id,
        "region_number": region_number, "source_file": source_file,
    ...
    with open(manifest_path, "w") as fh: json.dump(m, fh, indent=2)


def _write_slide_manifest(path: Path, *, pipeline: str, sample_id: str,
    source_file: str, n_regions: int, regions: List[Dict[str, Any]]) -> None:
    ...
    with open(path, "w") as fh:
        json.dump({"generated_at": datetime.utcnow().isoformat()+"Z",
            "pipeline": pipeline, "sample_id": sample_id,
            "source_file": source_file, "n_regions": n_regions,
