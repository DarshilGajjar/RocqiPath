...
class RegistrationQualityError(RegistrationError):
    """Report registration error above its configured quality threshold.

    The threshold is ``max_acceptable_error_um`` in micrometres.
    """
    ...
    def __init__(self, error_um: float, threshold_um: float) -> None:
        ...
        super().__init__(
            f"Registration error {error_um:.2f} µm exceeds threshold {threshold_um:.2f} µm."
        )
...
class DependencyError(WSIProcessingError, ImportError):
    ...
    Examples
    --------
    >>> raise DependencyError("valis", "pip install rocqipath[valis]")
    """
    ...
    def __init__(self, package: str, install_hint: str = "") -> None:
        ...
        install_hint : str, optional
            ...
            ``"pip install rocqipath[stain]"``. When omitted, the message
        ...
        super().__init__(f"Optional dependency '{package}' is not installed.{hint}")
