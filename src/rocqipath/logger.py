...
``rocqipath`` library and every script that builds on it.

This module is entirely self-contained — it has no imports from other
``rocqipath`` submodules, so it can be imported first, before anything
...
────────────────────────────────────────────────────────────────────────────────
  ...
  from rocqipath import logger as L
  L.print_banner()
...
────────────────────────────────────────────────────────────────────────────────
  ...
  LOGGER HELPERS ──────────────────────────────────────────────────────────────
  get_logger(name)          child logger namespaced as rocqipath.<name>
...
try:
    from rocqipath import __version__ as _PKG_VERSION
except Exception:
    _PKG_VERSION = "?"
...
"""
Shared Rich ``Console`` for the entire ``rocqipath`` library.
Use instead of ``print()`` so all output respects the project theme.

    from rocqipath.logger import console
...
"""
Loguru logger for the entire ``rocqipath`` library.
...
def track(
    ...
    Simple for-loop progress bar — drop-in replacement for ``tqdm``.
        ...
        from rocqipath.logger import track
...
# ══════════════════════════════════════════════════════════════════════════════
# SELF-TEST   python -m rocqipath.logger
# ══════════════════════════════════════════════════════════════════════════════
...
def _demo() -> None:
    ...
    get_logger("core").info("Child logger: [bold]rocqipath.core[/bold]")
    get_logger("alignment").info("Child logger: [bold]rocqipath.alignment[/bold]")
    set_log_level("INFO")
    ...
    print_code(
        "from rocqipath.logger import print_banner, track, Timer\nprint_banner()",
        language="python",
        title="Quick-start",
    )
    ...
    print_rule()
    print_done(f"Self-test complete  |  rocqipath {_PKG_VERSION}")
