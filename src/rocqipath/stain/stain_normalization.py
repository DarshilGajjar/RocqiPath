"""
rocqipath.stain.stain_normalization
===================================
...
Train a normaliser and save its weights::
    ...
    from rocqipath.stain.stain_normalization import (
...
Single-image use, without the batch workflow::
    ...
    from rocqipath.stain.stain_normalization import get_normalizer
...
CLI
---
    python -m rocqipath.stain.stain_normalization --mode train -i ./patches -o ./results --n_type macenko
    python -m rocqipath.stain.stain_normalization --mode apply -i ./patches -o ./results --n_type macenko
"""
...
# ---------------------------------------------------------------------------
# Logging — reuse the shared rocqipath logger; fall back gracefully
# ---------------------------------------------------------------------------

try:
    from rocqipath.logger import (
        logger,
        print_banner,
        print_section,
        print_step,
    ...
    logger = _sl.getLogger("rocqipath.stain.stain_normalization")  # type: ignore[assignment]

    def print_banner(*a: Any, **k: Any) -> None:
        """No-op fallback for ``rocqipath.logger.print_banner``.
        ...
        :mod:`rocqipath.logger` itself fails to import (unexpected, but
    ...
    def print_section(title: str) -> None:
        """Fallback for ``rocqipath.logger.print_section`` — logs ``title`` at INFO."""
        logger.info(title)

    def print_step(label: str, msg: str = "") -> None:
        """Fallback for ``rocqipath.logger.print_step`` — logs ``[label] msg`` at INFO."""
        logger.info(f"[{label}] {msg}")

    def print_done(msg: str) -> None:
        """Fallback for ``rocqipath.logger.print_done`` — logs ``msg`` at INFO, prefixed ``DONE:``."""
        logger.info(f"DONE: {msg}")

    def print_warn(msg: str) -> None:
        """Fallback for ``rocqipath.logger.print_warn`` — logs ``msg`` at WARNING."""
        logger.warning(msg)

    def print_error(msg: str) -> None:
        """Fallback for ``rocqipath.logger.print_error`` — logs ``msg`` at ERROR."""
        logger.error(msg)

    def print_info(msg: str) -> None:
        """Fallback for ``rocqipath.logger.print_info`` — logs ``msg`` at INFO."""
        logger.info(msg)

    def print_counts(ok: int, fail: int, label: str = "") -> None:
        """Fallback for ``rocqipath.logger.print_counts``.
        ...
        :func:`rocqipath.logger.print_counts` produces.
        """
        logger.info(f"{label} ok={ok} fail={fail}")

    def print_summary_table(rows: list, title: str = "") -> None:
        """Fallback for ``rocqipath.logger.print_summary_table``.
        ...
        :func:`rocqipath.logger.print_summary_table` produces.
        """
        logger.info(title)
        for k, v in rows:
            logger.info(f"  {k}: {v}")
    ...
    def track(iterable: Any, description: str = "") -> Any:
        """Fallback for ``rocqipath.logger.track`` — returns ``iterable`` unmodified.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--n_type",
        required=True,
        choices=sorted(_NORMALISER_REGISTRY),
...
def main(argv: Optional[List[str]] = None) -> int:
    """Entry point for ``python -m rocqipath.stain.stain_normalization``.
    ...
    int
        ...
        :class:`~rocqipath.exceptions.ConfigurationError`,
        :class:`~rocqipath.exceptions.ExtractionError`, or
        :class:`~rocqipath.exceptions.DependencyError` was raised and
