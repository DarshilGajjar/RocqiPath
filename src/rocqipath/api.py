...
Patch extraction (single slide, no registration)::
    ...
    from rocqipath.api import extract_patches_single
...
Grid-map export::
    ...
    from rocqipath.api import export_grid_map, export_paired_grid_maps
...
WSI thumbnail export::
    ...
    from rocqipath.api import export_wsi_thumbnails
...
Patch-pair visualisation::
    ...
    from rocqipath.api import visualize_patch_pairs
...
import shutil
import tempfile
from rocqipath.logger import logger  # noqa: E402
...
try:
    from rocqipath.visualization import plot_selector_map, view_pairs

    _HAS_VISUALIZATION = True
except ImportError as _e:
    plot_selector_map = view_pairs = None  # type: ignore[assignment]
    _HAS_VISUALIZATION = False
    _VISUALIZATION_IMPORT_ERROR = _e
...
def _require_core() -> None:
    """Raise if rocqipath.registration.core (and its dependencies) isn't available.

    Called at the top of every :mod:`rocqipath.api` function that needs
    :class:`~rocqipath.registration.core.WSIRegistrar` — a guard clause
    ...
    ------
    RuntimeError
        If the module-level import of ``rocqipath.registration.core``
    ...
    """
    if not _HAS_CORE or not _HAS_VISUALIZATION:
        reasons = []
        if not _HAS_CORE:
            reasons.append(str(_CORE_IMPORT_ERROR))
        if not _HAS_VISUALIZATION:
            reasons.append(str(_VISUALIZATION_IMPORT_ERROR))
        raise RuntimeError(
            "Grid-map dependencies are unavailable. Install 'rocqipath[orb,viz]'. "
            f"Import error: {'; '.join(reasons)}"
        )
...
def generate_single_grid_map_for_slide(
    ...
    Validates the file extension and path before delegating OpenSlide
    compatibility checks to ``WSIRegistrar``, then runs
    ``WSIRegistrar.generate_grid_map()`` and ``plot_selector_map()``.
...
def export_grid_map(
    ...
    *,
    ...
    ::
        ...
        from rocqipath.api import export_grid_map
    ...
    success, map_path, reason = generate_single_grid_map_for_slide(wsi_path, output_dir, cfg)
    if not success:
...
def export_paired_grid_maps(
    ...
    *,
    ...
    Pairing uses :func:`~rocqipath.utils.find_hne_ihc_pairs_by_suffix`
    ...
    ::
        ...
        from rocqipath.api import export_paired_grid_maps
    ...
    for pair in pairs:
        ...
        hne_ok, hne_map, hne_reason = generate_single_grid_map_for_slide(hne_path, single_dir, cfg)
        if not hne_ok:
            logger.warning(f"SKIP {suffix}: H&E grid map failed — {hne_reason}")
            skipped += 1
            continue

        ihc_ok, ihc_map, ihc_reason = generate_single_grid_map_for_slide(ihc_path, single_dir, cfg)
        if not ihc_ok:
...
def visualize_patch_pairs(
    ...
    Wraps :func:`rocqipath.visualization.view_pairs`.
