...
def enhance_ihc_thumbnail(rgb: np.ndarray, cfg: CoreExtractionConfig) -> np.ndarray:
    ...
    enhanced = cv2.cvtColor(cv2.merge([clahe.apply(lightness), a, b]), cv2.COLOR_LAB2RGB)
...
def discover_pairs(
    input_dir: str,
    use_suffix_pairing: bool = False,
    biomarker: str = "",
    stain_labels: Optional[List[str]] = None,
        ...
        if info is None:
            ...
            for label in sorted(custom_labels, key=len, reverse=True):
                ...
                if token and token in normalized_name:
                    ...
                    break
        if info is None:
            logger.warning(f"Unrecognised stain, skipping: {path.name}")
            continue
...
def extract_stain_cores(
    ...
    if cached_rgb is not None:
        ...
        ihc_boxes = _detect_regions(
            thumb, cfg.min_area_fraction, cfg.only_circles, cfg.min_circularity
        )
        logger.debug(
            f"{stain_label} | per-stain Otsu → {len(ihc_boxes)} (H&E: {len(he_rel_boxes)})"
        )
        if len(ihc_boxes) == 0:
            logger.warning(
                f"{stain_label} | 0 cores — {'fallback to H&E' if cfg.fallback_to_he else 'skipping'}"
            )
            ...
            logger.warning(
                f"{stain_label} | count mismatch IHC={len(ihc_boxes)} H&E={len(he_rel_boxes)} → fallback"
            )
    ...
    for idx, box in enumerate(active_boxes):
        ...
        if cfg.box_scale != 1.0:
            cx = x + w // 2
            cy = y + h // 2
            w = int(w * cfg.box_scale)
            h = int(h * cfg.box_scale)
            x = cx - w // 2
            y = cy - h // 2
        ...
        if cfg.skip_existing and _region_outputs_exist(core_dir, tag):
            logger.info(f"  SKIPPED  {stain_label}/{tag}")
            skipped += 1
...
def run_core_extraction_pipeline(
    ...
    if not pairs:
        logger.error(f"No WSI files found in {input_dir}")
        return

    n = len(pairs)
    logger.info(f"Core extraction pipeline — {n} block(s)")
    if not process_all:
        logger.info(f"Target stains: {sorted(target_stains)}")
    ...
    for i, (sample_id, stains) in enumerate(pairs.items(), 1):
        ...
        for lbl, info in selected:
            ...
            try:
                ns, nsk, mfs = extract_stain_cores(
                    info["path"],
                    lbl,
                    sample_id,
                    he_boxes,
                    out,
                    cfg,
                    cached_rgb=(he_thumb if lbl == "HnE" else None),
                )
                _write_slide_manifest(
                    Path(out) / f"{sample_id}_{lbl}_manifest.json",
                    pipeline="core",
                    sample_id=sample_id,
