"""Focused regression coverage for helpers relocated into ``rocqipath.io``."""
...
import rocqipath.io as io_package
from rocqipath.alignment.registrar import WSIRegistrar
from rocqipath._internal.geometry import region_bbox, transform_coords
from rocqipath.io.images import imread_rgb, imwrite_rgb
from rocqipath.io.naming import extract_sample_id
from rocqipath.io.vips import rgb_ome_xml


def test_former_utils_exports_remain_public() -> None:
    """Every name the former ``rocqipath.utils`` exported is public in ``rocqipath.io``."""
    assert set(io_package.__all__) >= {
        "detect_wsi_format",
